...
(function() {
  ...
  var ResultHandler = CKEDITOR.tools.createClass({
    ...
    proto : {
      ...
      getSelectedComponents: function () {
        ...
        for (var i = 0, l = inputs.count(); i < l; i++) {
          ...
          results.push({
            ...
            serviceChannelId: input.getAttribute('data-service-channel-id')
          });
        }
      ...
      renderComponent: function (component) {
        ...
        article.setAttribute('data-type', 'kunta-api-service-location-component');
        article.setAttribute('data-component', component.type);
        article.setAttribute('data-service-channel-id', component.serviceChannelId);
      ...
      loadPending: function (document) {
        ...
        for (var i = 0, l = pendingElements.count(); i < l; i++) {
          ...
          var serviceChannelId = pendingElement.getAttribute('data-service-channel-id');
          var params = {
            'action': 'kunta_api_render_service_location_component',
          ...
          ajaxLoader.doPost(ajaxurl, params, CKEDITOR.tools.bind(function (status, response) {
            if (status >= 200 && status <= 299) {
              ...
              for (var i = children.count() - 1; i >= 0; i--) {
                ...
                if ((child.type == CKEDITOR.NODE_ELEMENT) && child.getAttribute('data-type') == 'kunta-api-service-location-component') {
  ...
  var ResponseRenderer = CKEDITOR.tools.createClass({
    ...
    proto : {
      ...
      _createServiceHtml: function (serviceLocationChannel) {
        
        var name = this._getLocalizedValueAndType(serviceLocationChannel.names, LOCALE, 'Name');
        var description = this._getLocalizedValueAndType(serviceLocationChannel.descriptions, LOCALE, 'Description');
        ...
        serviceContainer.append(this._createCheckboxField(serviceLocationChannel.id, 'name', 'Nimi', name));
        serviceContainer.append(this._createCheckboxField(serviceLocationChannel.id, 'description', 'Kuvaus', description));
      ...
      _createCheckboxField: function (serviceChannelId, type, title, value) {
        var inputElement = new CKEDITOR.dom.element('input');
        inputElement.setAttribute('type', 'checkbox');
        inputElement.setAttribute('data-component-type', type);
        inputElement.setAttribute('data-service-channel-id', serviceChannelId);
  ...
  CKEDITOR.dialog.add('kunta-api-service-location-embed', function(editor) {
    return {
      title : 'Upota palvelupisteen tietoja',
      minWidth : 768,
      minHeight : 400,
      contents : [{
        id : 'embed',
